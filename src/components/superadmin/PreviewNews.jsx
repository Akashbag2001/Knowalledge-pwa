      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="w-[90%] max-w-5xl bg-[#1E1E1E] text-gray-100 rounded-2xl shadow-2xl border border-gray-800 overflow-hidden"
          >
            {/* Header */}
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-700 bg-[#2B2B2B]">
              <h2 className="text-2xl font-extrabold text-[#1f4edb]">📰 News Preview</h2>
              <button
                onClick={onClose}
            ...
            <div className="p-6 space-y-6 overflow-y-auto max-h-[75vh]">
              ...
              {/* Subheading */}
              {formData.subHeading && (
                <h2 className="text-2xl italic font-semibold text-gray-300 mb-6">
              ...
              {formData.smallContent && (
                <p className="text-gray-300 text-lg mb-4 bg-[#2B2B2B]/30 p-4 rounded border border-gray-700">
              ...
              {/* Topics & Metadata */}
              <div className="mt-6 border-t border-gray-700 pt-4 text-sm text-gray-400 flex flex-wrap gap-4">
                ...
                {formData.topics?.length > 0 && (
                  <span>
                    ...
                    {formData.topics.map((t, i) => (
                      <span
                        key={i}
                        className="px-2 py-1 rounded-full bg-[#1E2D5B] text-white text-xs mr-1"
                      >
                        {t}
                      </span>
                    ))}
            ...
            <div className="px-6 py-4 border-t border-gray-700 bg-[#2B2B2B] text-right">
              <button
                onClick={onClose}
                className="px-6 py-2 rounded-lg bg-[#1E2D5B] hover:bg-[#253b7a] text-white font-semibold transition"
              >
                Close Preview
              </button>
            </div>
