...
const AddNewTriviaPage = () => {
    ...
    const handleImageChange = (e) => {
        ...
        if (currentImageCount + files.length > maxAllowed) {
            toast.warning(`You can upload only ${maxAllowed} image${maxAllowed > 1 ? "s" : ""} for ${maxAllowed} card${maxAllowed > 1 ? "s" : ""}`);
            return;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.subCards.some((card) => !card.heading.trim() || !card.subHeading.trim() || !card.content.trim())) {
        ...
        try {
            ...
            // Append subCards as JSON string (matching your Postman format)
            ...
            if (response.success) {
                ...
                // Navigate to trivia list or wherever you want
                // setTimeout(() => {
